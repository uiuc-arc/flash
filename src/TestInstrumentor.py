...
tensorflow_import = ast.Import(names=[ast.alias(name='tensorflow', asname='tf')])
...
class TestInstrumentor(ast.NodeTransformer):
            ...
            Assign(targets=[Name(id=varname, ctx=Store())], value=Call(
                func=Attribute(value=Attribute(value=Name(id='np', ctx=Load()), attr='random', ctx=Load()),
                    ...
                    value=Call(func=Attribute(value=Name(id='np', ctx=Load()), attr='iinfo', ctx=Load()),
                               args=[Attribute(value=Name(id='np', ctx=Load()), attr=seed_size, ctx=Load())],
                               keywords=[]),
                    attr='max', ctx=Load())], keywords=[])),
            Expr(value=Call(
                func=Attribute(value=Name(id=lib_alias, ctx=Load()), attr=lib_func, ctx=Load()),
    ...
    def _create_fixture(self):
        ...
        if 'numpy' in self.dependencies:
            body = body + self._generate_seed_stub('npseed', 'np.random', 'seed', 'int32')
        if 'torch' in self.dependencies:
            body = body + self._generate_seed_stub('torchseed', 'torch', 'manual_seed', 'int64')
        if 'tensorflow' in self.dependencies:
            trypart = self._generate_seed_stub('tfseed', 'tf', 'set_random_seed', 'int64')
            exceptpart = self._generate_seed_stub('tfseed', 'tf.random', 'set_seed', 'int64')
    ...
    def instrument(self):
        if self.assert_spec_base is not None and self.assert_spec_base.test.filename != self.assert_spec.test.filename:
            with open(self.assert_spec_base.test.filename) as file:
                ...
                pos = [i for i in range(len(modified.body)) if isinstance(modified.body[i], ast.ClassDef)]
                if len(pos) == 0:
                    pos = [0]
                pos = pos[0]
    ...
    def write_file(self):
        ...
        shutil.copy(self.assert_spec.test.filename, self.assert_spec.test.filename + ".bak")
        ...
        if self.modified_ast_base is not None:
            shutil.copy(self.assert_spec_base.test.filename, self.assert_spec_base.test.filename + ".bak")
            with open(self.assert_spec_base.test.filename, 'w') as file:
    ...
    def restore_file(self, savedir=None):
        ...
        shutil.move(self.assert_spec.test.filename + ".bak", self.assert_spec.test.filename)
        if self.modified_ast_base is not None:
            shutil.move(self.assert_spec_base.test.filename + ".bak", self.assert_spec_base.test.filename)
                ...
                if isinstance(node.value.func, ast.Name):
                    ...
                    if AssertType.get_assert_type(node.value.func.id) is not None:
                        ...
                        if node.value.func.id in ['assertTrue', 'assertFalse'] and isinstance(node.value.args[0],
                                                                                              ast.Compare):
                            printstmt = self.get_print_stmt(node.value.args[0].left)
